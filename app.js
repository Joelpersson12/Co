...
/* =========================================================
   Correction flow ("Rätta med ny post")
   ========================================================= */
function correctEntry(id) {
  const orig = findLedgerById(id);
  if (!orig) return;
  const msg = `Detta förbereder en NY post som stornerar (nollställer) verifikation #${orig.verNr}.\n\n` +
    `Den ursprungliga posten ändras eller tas INTE bort — så kräver bokföringslagen. ` +
    `Du kontrollerar och bokför rättelsen på nästa sida.\n\nFortsätta?`;
  ...
  formDraft = {
    ...
    bannerText: `🔧 Rättar verifikation #${orig.verNr} — beloppet är förifyllt med motsatt tecken (stornering). Kontrollera och bokför.`,
  };
  navTo('kvitton');
  applyFormDraft();
}
...
function setupReceiptForm() {
  ...
  form.addEventListener('submit', async (e) => {
    e.preventDefault();
    const v = readFormInputs(form);
    const isCorrection = !!(formDraft && formDraft.correctionOf);
    if (v.gross === 0) { toast('Fyll i ett belopp'); return; }
    if (!isCorrection && v.gross < 0) { toast('Beloppet ska vara positivt. Använd "Rätta" på posten du vill korrigera.'); return; }
    const isCapital = v.type === 'insattning' || v.type === 'uttag';
    ...
    const entry = addLedgerEntry({
      ...
      correctionOf: formDraft && formDraft.correctionOf ? formDraft.correctionOf : null,
    });

    if (formDraft && formDraft.calcId) {
      const c = state.calculations.find(x => x.id === formDraft.calcId);
      if (c) c.bookedId = entry.id;
    }
    const wasDraft = !!formDraft;
    formDraft = null;
    ...
    save();
    renderAll();
    toast(`Verifikation #${entry.verNr} bokförd${wasDraft ? '' : ''} ✓`);
  });
